...
impl StorageNodeServer {
    ...
    pub fn create_api(&self) -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
        ...
        let routes = self.store_chunk(storage_path.clone())
    ...
    fn store_chunk(&self, storage_path: PathBuf) -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
        warp::path!("chunks")
            .and(warp::post())
            .and(warp::body::bytes())
    ...
    fn retrieve_chunk(&self, storage_path: PathBuf) -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
    ...
    fn list_chunks(&self, storage_path: PathBuf) -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
        warp::path!("chunks")
            .and(warp::get())
            .and(warp::any().map(move || storage_path.clone()))
...
async fn list_stored_chunks(storage_path: &PathBuf) -> Result<Vec<String>> {
    ...
    while let Some(entry) = entries.next_entry().await.context("Failed to read directory entry")? {
        if entry.file_type().await.context("Failed to get file type")?.is_file() {
...
mod tests {
    ...
    async fn test_store_and_retrieve_chunk() {
        ...
        // Store chunk
        store_chunk_data(&storage_path, &expected_hash, test_data).await.unwrap();

        // Retrieve chunk
        let retrieved_data = load_chunk_data(&storage_path, &expected_hash).await.unwrap();
    ...
    async fn test_list_chunks() {
        ...
        store_chunk_data(&storage_path, &hash1, test_data1).await.unwrap();
        store_chunk_data(&storage_path, &hash2, test_data2).await.unwrap();
